...
// Function to add colleague's name, time zone, and local time to the list
function addColleague() {
    ...
    // Fetch current time data from WorldTimeAPI for the selected time zone
    fetch(`http://worldtimeapi.org/api/timezone/${selectedTimeZone}`)
        .then(response => response.json())
        .then(data => {
            const localTime = new Date(data.datetime);
            const localTimeString = localTime.toLocaleTimeString(undefined, { hour: 'numeric', minute: '2-digit', hour12: true }); // Convert UTC time to local time string in AM/PM format
            const colleagueItem = document.createElement("li");
            colleagueItem.innerHTML = `${colleagueName}: ${selectedTimeZone} (${localTimeString}) <button onclick="deleteColleague(this)">Delete</button>`;
